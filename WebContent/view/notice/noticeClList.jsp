...
<jsp:include page="../main/mycarClHeader.jsp"></jsp:include>
  ...
  <ul class="pagination">
		...
		<%}else{ %>
	<li class="page-item">
      <a class="page-link" href="noticeClList.no?page=<%=startPage-1 %>" aria-label="Previous" title="<%=startPage-1 %> 페이지로 이동">
		...
		<%}else{ %>
		<li class="page-item" aria-current="page"><a class="page-link" href="noticeClList.no?page=<%=a %>" title="<%=a %>페이지로 이동"><%=a %>
		</a></li>
		<%} %>
		<%} %>
		...
		<%}else{ %>
	<li class="page-item">
		<a class="page-link" href="noticeClList.no?page=<%=endPage+1 %>" aria-label="Next" title="<%=endPage+1 %> 페이지로 이동">
